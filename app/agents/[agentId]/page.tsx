    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div
              className={cn(
                ...
                agent.status === "online" && "bg-accent/10 text-accent",
                agent.status === "offline" && "bg-destructive/10 text-destructive",
                agent.status === "processing" && "bg-secondary/10 text-secondary"
              )}
            >
              <div
                className={cn(
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-lg shadow-black/5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Latest insights from this agent
          </p>
          <div className="mt-4 space-y-3">
            {agent.recentAnalysis.map((analysis, index) => (
              <div
                key={index}
      ...
      <div className="rounded-xl border border-border bg-card p-6 shadow-lg shadow-black/5">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            { label: "1 Hour", value: agent.accuracy - 3 },
            { label: "4 Hours", value: agent.accuracy },
            { label: "Daily", value: agent.accuracy + 2 },
            { label: "Weekly", value: agent.accuracy + 5 },
          ].map((metric) => (
      ...
      <div className="rounded-lg border border-dashed border-border bg-muted/20 p-4">
        <p className="text-sm text-muted-foreground">
          {/* PLACEHOLDER: Real API integration would fetch live agent data from your backend */}
