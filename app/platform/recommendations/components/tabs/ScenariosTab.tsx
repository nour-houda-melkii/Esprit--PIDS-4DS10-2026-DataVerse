...
import { Pair, LivePairData, Scenario } from "../../lib/types";
import { sc, fmt } from "../../lib/utils";
import { DecisionBadge } from "../../components/ui/DecisionBadge";
import { CONVICTION_STATIC, PRICE } from "../../lib/constants"; 
...
const FALLBACK_SCENARIOS: Scenario[] = [
  {
    ...
    pairEffects: {
      ...
      "EUR/JPY": { direction:  0, magnitude: 0.10 },  // changed from EUR/GBP
  ...
  },
  {
    ...
    pairEffects: {
      ...
      "EUR/JPY": { direction:  1, magnitude: 0.50 },  // changed from EUR/GBP
  ...
  },
  {
    ...
    pairEffects: {
      ...
      "EUR/JPY": { direction:  0, magnitude: 0.15 },  // changed from EUR/GBP
      "GBP/JPY": { direction:  1, magnitude: 0.45 },
    },
  },
];

// Static data for fallback conviction/price - use our shared pairs


// Helper to compute outcome
...
// Hook for dynamic scenarios (same as before, just using shared types)
...
export function ScenariosTab({
  ...
  const allOut = scenarios.map(s => ({
    name: s.name.split(" ")[0],
    conf: outcome ? Math.round(computeOutcome(pair, sig, growth, s, capital).confidence * 100) : 0,
    surv: outcome ? Math.round(computeOutcome(pair, sig, growth, s, capital).survival * 100) : 0,
  }));
      ...
      <div className="rounded-3xl border border-blue-500/20 bg-blue-500/[0.04] p-5">
        ...
        {error && !loading && (
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-rose-500/20 bg-rose-500/[0.06] px-3 py-2">
            ...
            <p className="text-[10px] text-rose-400">AI generation failed — showing static fallbacks. {error.includes("500") ? "LLM API error." : error}</p>
          </div>
        )}
      </div>
                    ...
                    {active && (
                      <>
                        <p className="text-[11px] text-slate-500 mt-2 leading-relaxed">
                          <span className="text-slate-400 font-semibold">Trigger: </span>{s.triggerEvent}
                        </p>
